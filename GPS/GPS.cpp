...
GPS::GPS(int pinTX, int pinRX) :message {'$', 'G', 'P', 'G', 'L', 'L'}, m_donneesUtilesRecuperees(true), valueRetrieved(false),
...
void GPS::recupDonneesBrut()
{
  ...
  else
  {
    ...
    {
      Serial.println("Donnees lues");
      Serial.println(m_recupDonneesBrut);
      m_recupDonneesBrut = "";
    }
  }
}
